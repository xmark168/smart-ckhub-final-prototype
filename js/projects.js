  ...
  function openOnboarding(item) {
    ...
    modal.addEventListener("submit", function (event) {
      ...
      if (!item.contractCode) {
        if (!window.CKHubContracts || !form.contractFile.files.length) {
          if (window.showToast) window.showToast("Tải file hợp đồng trước khi lưu.");
          return;
        }
        window.CKHubContracts.createPrimary(item, {
          ...
          evidence: form.contractFile.files[0].name,
        });
        addActivity(item, "file-check-2", "Đã hoàn tất hợp đồng chính", item.contractCode + " · " + form.contractFile.files[0].name);
      }
      item.onboarding = {
