(function () {
  ...
  function openOnboarding(item) {
    var data = item.onboarding || {}, items = onboardingItems(item), completed = items.filter(function (entry) { return entry.ready; }).length, contractSection = item.contractCode ? '<section class="onboarding-form-section"><div class="onboarding-section-head"><div><span class="onboarding-owner">Hợp đồng</span><b>Hợp đồng chính</b><p>Đã liên kết ' + esc(item.contractCode) + '.</p></div><span class="onboarding-form-status complete">Đã liên kết</span></div></section>' : '<section class="onboarding-form-section"><div class="onboarding-section-head"><div><span class="onboarding-owner">Hợp đồng</span><b>Hoàn tất hợp đồng chính</b><p>Lưu hợp đồng tại Google Drive và liên kết trực tiếp với dự án.</p></div><span class="onboarding-form-status">Cần tạo</span></div><div class="onboarding-contract-grid"><label class="field onboarding-field"><span>Số hợp đồng</span><input name="contractCode" required value="HĐ-2026-' + esc(item.code.slice(-3)) + '"></label><label class="field onboarding-field"><span>Số chu kỳ</span><input name="contractCycles" type="number" min="1" required value="6"></label><label class="field onboarding-field"><span>Ngày bắt đầu hợp đồng</span><input name="contractStart" type="date" required value="2026-10-01"></label><label class="field onboarding-field"><span>Hạn thanh toán</span><input name="contractPaymentDue" type="date" required value="2026-10-05"></label><label class="field onboarding-field onboarding-contract-value"><span>Giá trị hợp đồng</span><input name="contractValue" type="number" min="0" required value="' + Number(item.servicePrice || 0) + '"></label><label class="field onboarding-field onboarding-drive"><span>Link hợp đồng trên Drive</span><input name="contractDrive" type="url" required placeholder="https://drive.google.com/..."></label><label class="field onboarding-field onboarding-upload"><span>Tệp đính kèm (tùy chọn)</span><input name="contractFile" type="file" accept=".pdf,.doc,.docx"></label></div></section>', modal = document.createElement("div");
    modal.className = "modal-backdrop show customer-modal";
    modal.innerHTML =
    ...
    modal.addEventListener("submit", function (event) {
      ...
      if (!item.contractCode) {
        if (!window.CKHubContracts || !form.contractDrive.value.trim()) {
          if (window.showToast) window.showToast("Bổ sung link hợp đồng trên Drive trước khi lưu.");
          return;
        }
        window.CKHubContracts.createPrimary(item, {
          code: form.contractCode.value.trim().toUpperCase(),
          cycles: form.contractCycles.value,
          start: form.contractStart.value,
          value: form.contractValue.value,
          paymentDue: form.contractPaymentDue.value,
          evidence: form.contractDrive.value.trim(),
          attachmentName: form.contractFile.files.length ? form.contractFile.files[0].name : "",
        });
        addActivity(item, "file-check-2", "Đã hoàn tất hợp đồng chính", item.contractCode + " · Đã liên kết Google Drive.");
      }
      item.onboarding = {
